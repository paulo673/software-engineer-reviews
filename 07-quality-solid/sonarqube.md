---
id: sonarqube
title: "SonarQube and Static Analysis"
area: quality-solid
difficulty: beginner
prerequisites: [code-smells]
related: [code-smells, ci-cd]
tags: [sonarqube, static-analysis, quality-gate, coverage, quality]
sources:
...
SonarQube é uma plataforma de análise estática que inspeciona o código sem executá-lo, apontando bugs prováveis, vulnerabilidades de segurança, code smells e duplicação, além de medir coverage de tests. Seu mecanismo central é o quality gate: um conjunto de critérios que o código precisa cumprir para ser aprovado no pipeline. Importa porque automatiza a fiscalização de quality, dando feedback objetivo e contínuo em vez de depender só de revisão manual.
...
- **Cobertura de tests**: percentual de linhas/branches exercitados por tests. Não garante quality dos tests, mas revela código não testado.
...
O **quality gate** é o coração do uso em CI/CD (ver [CI/CD](../06-docker-k8s-cicd-azure/ci-cd.md)): define condições como "nenhum bug novo", "coverage do código novo acima de 80%", "duplicação abaixo de X". Se o código não passa, o pipeline falha. O foco moderno é no **clean as you code**: aplicar critérios rígidos ao código novo ou alterado, em vez de exigir corrigir todo o legado de uma vez.
...
O SonarQube usa um scanner que processa o código e, idealmente, os relatórios de coverage gerados pelos tests, enviando os resultados a um servidor que armazena histórico, calcula métricas e avalia o quality gate. Para projetos .NET, o `dotnet-sonarscanner` instrumenta o build: você inicia a análise, compila, roda os tests coletando coverage (por exemplo, com Coverlet) e finaliza a análise, que então publica os resultados.
...
- Quality gates rígidos elevam o padrão, mas se calibrados mal (coverage irreal, regras excessivas) geram atrito e burlas (tests vazios só para subir coverage).
- Cobertura é uma métrica útil, porém enganosa se tratada como objetivo final: 100% de coverage com asserções fracas não garante quality.
...
- Tratar coverage como meta a maximizar a qualquer custo: leva a tests sem asserção real, métrica alta e quality baixa.
...
Use análise estática em todo projeto sério, integrada ao pipeline, com um quality gate focado no código novo (clean as you code) e coverage como sinal, não como dogma. Combine SonarQube (visão histórica, segurança, dívida) com Roslyn analyzers (feedback imediato no build). Trate os security hotspots com revisão humana. Não substitua a revisão de código nem o julgamento de design pela ferramenta; use-a para automatizar o que é mecânico e liberar a revisão humana para o que exige contexto.
...
<details><summary>Resposta</summary>Um conjunto de critérios (sem bugs novos, coverage mínima do código novo, duplicação máxima) que o código precisa cumprir; se falhar, o pipeline falha.</details>

3. O que significa "clean as you code"?
<details><summary>Resposta</summary>Aplicar os critérios de quality ao código novo ou alterado, em vez de exigir corrigir todo o legado de uma vez, evitando travar o time e melhorando de forma incremental.</details>

4. Por que coverage de tests pode enganar?
<details><summary>Resposta</summary>Porque mede linhas exercitadas, não a quality das asserções; é possível ter coverage alta com tests fracos que não verificam comportamento de fato.</details>
...
```mermaid
flowchart LR
    CODE[Código + coverage] --> SCAN[Scanner / analisadores]
    SCAN --> ISS[Issues: bugs, vulnerabilidades, smells]
    SCAN --> MET[Métricas: coverage, duplicação, complexidade]
    ISS --> QG{Quality gate}
    MET --> QG
