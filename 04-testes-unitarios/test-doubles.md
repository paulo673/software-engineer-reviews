---
id: test-doubles
title: "Test Doubles: Dummy, Stub, Fake, Mock e Spy"
area: testes-unitarios
difficulty: intermediario
prerequisites: [padrao-aaa]
related: [padrao-aaa, ferramentas-teste]
tags: [testes, mocks, stubs, fakes, dublês, isolamento]
sources:
...
Test doubles (dublês de teste) são objetos que substituem dependências reais durante um teste, para isolar o código sob teste de banco, rede, relógio e outros serviços. O termo guarda-chuva, de Gerard Meszaros, abrange cinco tipos com propósitos distintos: dummy, stub, fake, mock e spy. A distinção principal está entre verificar estado (stub/fake) e verificar interação (mock/spy), e confundi-los leva a testes frágeis ou pouco significativos.
...
- **Fake**: uma implementação funcional, porém simplificada, leve demais para produção mas real o suficiente para o teste. O exemplo clássico é um repositório em memória que substitui o banco, ou um `TimeProvider` controlável. Tem comportamento de verdade.
...
Em frameworks como Moq e NSubstitute (ver [ferramentas de teste](ferramentas-teste.md)), o mesmo objeto pode atuar como stub (configurando retornos) e como mock (verificando chamadas), então os termos se misturam na prática, mas a intenção do teste continua sendo estado ou interação.
...
Por isso essas bibliotecas exigem que o membro seja substituível: interfaces, métodos `virtual` ou `abstract`. Um método não virtual de classe concreta não pode ser interceptado, o que é uma das razões para programar contra interfaces (ver [SOLID](../07-qualidade-solid/solid.md), inversão de dependência).
...
- Verificação de estado (stub/fake) produz testes mais robustos a refatoração: você testa o resultado, não o como. O risco é não cobrir interações que importam (efeitos colaterais como publicar evento).
- Verificação de interação (mock) é necessária quando o efeito é a própria chamada (enviar e-mail, publicar mensagem), mas acopla o teste à implementação: mudar a forma de colaborar quebra o teste mesmo sem mudar o comportamento observável (testes frágeis).
- Fakes dão testes realistas e rápidos, mas precisam ser mantidos para refletir o contrato real; um fake que diverge do real mascara bugs.
- Mockar demais é um cheiro: pode indicar que o SUT tem dependências em excesso.

## Pegadinhas e erros comuns

- Usar mock (verificar interação) quando bastaria verificar estado, criando testes frágeis que quebram a cada refatoração interna.
- Mockar tipos que você não possui (bibliotecas externas) diretamente, em vez de envolvê-los numa abstração própria.
...
1. Qual a distinção conceitual mais importante entre os tipos de test double?
...
4. Por que verificar interação pode gerar testes frágeis?
