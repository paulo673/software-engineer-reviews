---
id: padrao-aaa
title: "Padrão AAA (Arrange, Act, Assert)"
area: testes-unitarios
difficulty: iniciante
prerequisites: []
related: [test-doubles, ferramentas-teste]
tags: [testes, aaa, unit-test, estrutura, boas-praticas]
sources:
...
- **Independência**: testes não dependem de ordem nem de estado compartilhado; cada um arranja o próprio cenário.
- **Determinismo**: o mesmo teste sempre dá o mesmo resultado, sem depender de relógio, rede ou aleatoriedade (use abstrações como `TimeProvider`, ver [novidades do .NET 8](../01-csharp-dotnet/novidades-dotnet8-csharp12.md)).
...
A maioria dos frameworks de teste em .NET (xUnit, NUnit, MSTest, ver [ferramentas de teste](ferramentas-teste.md)) descobre métodos marcados com atributos (`[Fact]`, `[Test]`) por reflection, instancia a classe de teste e executa cada método isoladamente, capturando exceções. Uma asserção que falha lança uma exceção específica do framework, que é registrada como falha sem interromper os outros testes.

No xUnit, a classe de teste é instanciada uma vez por teste, o que reforça a independência: estado de instância não vaza entre testes. Inicialização comum vai no construtor (o "arrange" compartilhado) e limpeza em `IDisposable.Dispose` ou `IAsyncLifetime`.
...
- AAA torna os testes legíveis e diagnosticáveis, com custo praticamente nulo; é convenção, não ferramenta.
- Insistir em um único Act por teste melhora o foco, mas pode aumentar o número de testes; o ganho de clareza compensa.
...
- Excesso de preparação (Arrange) indica que o SUT tem dependências demais, um sinal de design (ver [SOLID](../07-qualidade-solid/solid.md)).
...
Use AAA como estrutura padrão de praticamente todo teste unitário, com nomes descritivos e um comportamento por teste. Use `[Theory]`/`[InlineData]` para cobrir variações de entrada sem duplicar código. Mantenha os testes independentes e determinísticos. Não há "quando evitar" o AAA em si; o que se evita é o contrário: testes sem estrutura, com múltiplos comportamentos e dependências de ordem.
...
<details><summary>Resposta</summary>Depender de relógio, rede, aleatoriedade, ordem de execução ou estado compartilhado. Evita-se com determinismo e isolamento, abstraindo o tempo e as dependências externas.</details>
