...
O problema N+1 acontece quando, para carregar uma lista de N entidades e seus relacionamentos, o código dispara 1 consulta para a lista mais N consultas adicionais, uma por entidade, para buscar os dados relacionados. O resultado é dezenas ou centenas de idas ao banco onde bastariam uma ou duas. É uma das causas mais comuns de lentidão com ORMs, e a correção é carregar os relacionamentos de forma antecipada.
...
- **Split queries**: quando o `Include` traz muitas coleções e o `JOIN` causa explosão cartesiana (linhas duplicadas), `AsSplitQuery` divide em uma consulta por coleção, evitando o produto cartesiano ao custo de mais idas ao banco (mas poucas, não N+1).
...
- Split query evita o produto cartesiano com poucas consultas extras, ao custo de não ser uma única ida ao banco e de possível inconsistência se os dados mudarem entre as consultas (sem transação).
...
- Não olhar o SQL gerado: a única forma confiável de detectar N+1 é inspecionar as consultas (logging do EF, ferramentas de profiling).
...
<details><summary>Resposta</summary>Porque carrega-se a lista com 1 consulta e, para cada uma das N entidades, dispara-se mais 1 consulta para os dados relacionados, totalizando N+1 idas ao banco.</details>
...
<details><summary>Resposta</summary>Trazendo os dados relacionados na mesma consulta via JOIN, reduzindo de N+1 para 1 ida ao banco.</details>
...
<details><summary>Resposta</summary>Inspecionando o SQL gerado via logging do EF ou ferramentas de profiling; é a única forma segura de ver as consultas repetidas.</details>
