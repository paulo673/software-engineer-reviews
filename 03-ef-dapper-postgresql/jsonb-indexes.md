---
id: jsonb-indexes
title: "JSONB Indexes in PostgreSQL"
area: ef-dapper-postgresql
difficulty: intermediate
prerequisites: [essential-sql]
related: [essential-sql, transaction-isolation]
tags: [postgresql, indexes, jsonb, btree, gin, performance]
sources:
...
Índices no PostgreSQL aceleram consultas evitando varrer a tabela inteira, com types diferentes para necessidades diferentes (B-tree para equality e faixa, GIN para conteúdo de coleções e JSONB). JSONB é o tipo binário para armazenar JSON com consultas e indexação eficientes. Entender qual índice usar e como consultar JSONB é o que separa um schema rápido de um lento quando há dados semiestruturados.

## Explicação detalhada

**Tipos de índice mais usados:**

- **B-tree** (padrão): para equality e faixa (`=`, `<`, `>`, `BETWEEN`, `ORDER BY`). Cobre a maioria dos casos com colunas escalares.
...
- **Hash**: só para equality; raramente preferível ao B-tree.
...
O GIN inverte a estrutura: para cada componente (elemento de array, key/value de JSONB), mantém a lista de linhas que o contêm. Por isso responde rápido a "quais linhas contêm X". A construção e atualização do GIN é mais cara que a do B-tree, então ele penaliza mais a escrita.
...
- JSONB dá flexibilidade de schema (dados semiestruturados, campos variáveis) sem migrations para cada atributo, mas perde parte das garantias e da clareza de colunas tipadas; abusar dele vira um database sem schema.
...
- Modelar tudo como JSONB e perder integridade referencial, types e clareza que colunas relacionais dariam.

## Quando usar e quando evitar

Use B-tree para a maioria das colunas filtradas, ordenadas ou usadas em joins. Use GIN para consultas de contenção em JSONB, arrays e full-text. Use índices parciais e compostos guiados pelas consultas reais, verificando com `EXPLAIN ANALYZE` (ver [Essential SQL](essential-sql.md)). Use JSONB para dados genuinamente semiestruturados ou de schema variável. Evite indexar colunas pouco usadas, evite JSONB para dados que são naturalmente relacionais e precisam de integridade e tipagem.

## Perguntas de auto-teste

1. Quando usar um índice GIN em vez de B-tree?
<details><summary>Resposta</summary>Para valores com múltiplos componentes (arrays, full-text, JSONB), respondendo perguntas de contenção/existência. B-tree serve equality e faixa em colunas escalares.</details>
...
flowchart TD
    ...
    T -->|equality / faixa escalar| BT[B-tree]
