...
title: "Transaction Isolation and MVCC in PostgreSQL"
area: ef-dapper-postgresql
difficulty: advanced
prerequisites: [essential-sql]
related: [essential-sql, jsonb-indexes, idempotency]
tags: [transaction, isolation, mvcc, postgresql, concurrency, acid]
sources:
...
O nível de isolation de uma transação define o quanto ela é protegida dos efeitos de transações concorrentes. A teoria classifica anomalias (dirty read, non-repeatable read, phantom read) e níveis (Read Uncommitted, Read Committed, Repeatable Read, Serializable) que as previnem progressivamente. O PostgreSQL implementa isolation com MVCC (multiversão), e seu padrão é Read Committed. Entender isso evita bugs de concorrência sutis e perda de dados sob carga.

## Explicação detalhada

A propriedade I de ACID é o isolation. Quanto mais alto o nível, menos anomalias, porém maior o custo de concorrência (mais bloqueios ou mais conflitos e retries).
...
**Níveis de isolation do padrão SQL e o que previnem:**
...
Definir nível de isolation explícito com EF Core:
...
2. Qual o nível de isolation padrão do PostgreSQL?
...
<details><summary>Resposta</summary>Com bloqueio pessimista (SELECT ... FOR UPDATE), controle otimista por coluna de versão, ou isolation Serializable com retry.</details>
