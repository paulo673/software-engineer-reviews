...
tags: [ef-core, migrations, schema, versioning, database]
sources:
...
- `dotnet ef database update`: aplica as migrations pendentes ao database.
...
O EF mantém no database uma tabela `__EFMigrationsHistory` com as migrations já aplicadas, para saber o que falta. É assim que ele sabe quais migrations rodar em cada ambiente.

**Aplicação em produção**: há três caminhos principais.

- **Script SQL** gerado e reviewed, aplicado pelo pipeline. É o mais seguro e auditável, recomendado para produção.
...
Cada migration carrega um snapshot do modelo (`ModelSnapshot`), uma representação completa do schema esperado após aquela migration. O diff é calculado entre o modelo das entidades e esse snapshot, não lendo o database. Por isso o snapshot precisa estar versionado e correto: se ele divergir do database real, o diff sai errado.
...
- Aplicar via script reviewed é seguro e auditável, mas exige um passo no pipeline. `Database.Migrate()` na inicialização é cômodo, porém arriscado em escala (corrida entre instâncias, acoplamento de schema ao boot).
...
- Editar o database manualmente e deixar o snapshot divergir: o próximo diff fica errado. Toda mudança de schema deve passar por migration.
...
Use migrations do EF Core sempre que o EF for o dono do schema, mantendo cada mudança versionada e aplicando em produção via script idempotente reviewed no pipeline. Para transformações de dados, escreva SQL explícito na migration. Evite alterações manuais no database fora das migrations, e em equipes onde DBAs controlam o schema com outra ferramenta, alinhe o processo para não haver duas fontes da verdade. Evite `Database.Migrate()` no boot em produção com múltiplas instâncias.

## Perguntas de auto-teste

1. O que o EF Core compara para gerar uma nova migration?
<details><summary>Resposta</summary>O modelo atual das entidades com o snapshot do modelo da última migration (não lê o database). O diff vira a nova migration.</details>

2. Para que serve a tabela `__EFMigrationsHistory`?
<details><summary>Resposta</summary>Registra quais migrations já foram aplicadas no database, permitindo ao EF saber o que ainda falta aplicar em cada ambiente.</details>
...
flowchart LR
    ...
    MIG -->|migrations script idempotente| SQL[Script reviewed]
