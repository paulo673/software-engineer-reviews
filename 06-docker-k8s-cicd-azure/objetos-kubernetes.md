...
**Deployment**: gerencia um conjunto de Pods idênticos (réplicas) por meio de um ReplicaSet. Você declara quantas réplicas quer e qual imagem usar; o Deployment garante esse número, recria Pods que morrem e faz **rolling updates**: ao mudar a imagem, sobe Pods novos e desce os antigos gradualmente, sem downtime, com rollback possível.

**Service**: dá um endereço de rede estável (nome DNS e IP virtual) para um conjunto de Pods selecionados por labels, balanceando o tráfego entre eles. Resolve o problema de os IPs dos Pods mudarem (ver [service discovery](api-gateway-service-discovery.md)). Tipos principais:

- **ClusterIP** (padrão): acessível só dentro do cluster.
- **NodePort**: expõe em uma porta de cada nó.
- **LoadBalancer**: provisiona um balanceador externo (na nuvem).

**ConfigMap e Secret**: externalizam configuração da imagem. ConfigMap guarda configuração não sensível (chave-valor, arquivos); Secret guarda dados sensíveis (senhas, tokens), codificados em base64 e com controle de acesso. Ambos são injetados nos Pods como variáveis de ambiente ou arquivos montados.
...
Outros objetos comuns: **Namespace** (isolamento lógico), **StatefulSet** (workloads com identidade e estado estáveis, como bancos), **DaemonSet** (um Pod por nó), **Job/CronJob** (tarefas pontuais ou agendadas), **HorizontalPodAutoscaler** (escala réplicas por métrica).
...
- Service ClusterIP é simples e interno; expor para fora exige LoadBalancer (custo de balanceador na nuvem) ou Ingress (mais configuração, mas roteamento HTTP rico e menos balanceadores).
...
- Hardcodar configuração e segredos na imagem em vez de usar ConfigMap/Secret, perdendo flexibilidade entre ambientes.
...
<details><summary>Resposta</summary>Mantém um número desejado de réplicas de Pods, recria os que morrem e realiza rolling updates (e rollback) ao mudar a imagem, sem downtime.</details>
