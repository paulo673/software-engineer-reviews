...
difficulty: beginner
prerequisites: []
related: [kubernetes-objects, ci-cd]
tags: [docker, image, container, layers, dockerfile]
sources:
...
Uma image é um template imutável e empacotado de uma aplicação e suas dependências; um container é uma instância em execução criada a partir de uma image. A relação é a mesma entre classe e objeto: a image define, o container executa. Entender a distinção, as camadas (layers) e a immutability é a base para empacotar aplicações de forma reprodutível e para usar Docker e Kubernetes corretamente.

## Explicação detalhada

Uma **image** é um pacote read-only que contém tudo que a aplicação precisa para rodar: o binário, o runtime (por exemplo, o .NET runtime), bibliotecas, arquivos e a configuração de inicialização. Ela é construída a partir de um `Dockerfile`, identificada por nome e tag (`myapp:1.2.0`) e por um digest (hash do conteúdo), e armazenada em um registry (Docker Hub, Azure Container Registry).

Um **container** é uma instância em execução de uma image. Quando você roda a image, o runtime cria um container: um processo isolado com seu próprio sistema de arquivos (a image mais uma camada gravável por cima), rede e limites de recursos. Você pode criar muitos containers da mesma image, todos partindo do mesmo estado definido por ela.

A diferença chave é **immutability**: a image não muda. O container tem uma camada de escrita efêmera; mudanças feitas nele (arquivos gravados em runtime) somem quando o container é removido, a menos que se use um volume para persistir. Por isso o princípio: containers devem ser **stateless** e descartáveis, com estado externo (database, volume, storage).
...
Uma image é composta de **camadas** empilhadas. Cada instrução do `Dockerfile` que altera o sistema de arquivos (`COPY`, `RUN`) cria uma camada read-only. As camadas são reaproveitadas entre imagens e em cache de build: se uma camada não mudou, o Docker reusa a do cache, acelerando builds e economizando espaço e transferência.

Ao criar um container, o runtime adiciona uma fina **camada gravável** no topo das camadas read-only da image, usando um filesystem de união (overlay). Leituras vêm das camadas da image; escritas vão para a camada do container (copy-on-write). Remover o container descarta essa camada.
...
Comandos básicos do ciclo image para container:
...
Os dois `run` criam containers distintos da mesma image.

## Tradeoffs

- Imagens imutáveis dão reprodutibilidade e versioning: o mesmo digest roda igual em qualquer lugar. O custo é repensar persistência (estado fora do container) e reconstruir/redeployar para mudar.
- Containers são leves e rápidos comparados a VMs, mas compartilham o kernel do host, oferecendo isolation mais fraco que a virtualização completa.
- Multi-stage builds reduzem tamanho e superfície de ataque da image final, ao custo de um Dockerfile um pouco mais elaborado.
...
- Guardar estado ou segredo dentro da image: imagens são distribuídas e versionadas; segredos não pertencem a elas. Use variáveis de ambiente e cofres de segredo.
...
- Usar a image do SDK em produção em vez do runtime, resultando em image enorme e com mais superfície de ataque.
- Confundir image e container: parar um container não apaga a image; remover a image não afeta containers já em execução até pararem.
- Usar a tag `latest` em produção, perdendo rastreabilidade de qual versão está rodando.

## Quando usar e quando evitar

Use imagens para empacotar a aplicação de forma reprodutível e versionada, com multi-stage build para imagens enxutas. Trate containers como efêmeros e stateless, mantendo estado em volumes ou serviços externos. Use tags de versão explícitas (não `latest`) em produção. Containers são a unidade natural de deploy para microsserviços orquestrados por [Kubernetes](kubernetes-objects.md). Evite colocar estado ou segredos na image e evite depender de mudanças feitas em runtime no container.

## Perguntas de auto-teste

1. Qual a relação entre image e container?
<details><summary>Resposta</summary>A image é um template imutável; o container é uma instância em execução criada a partir dela. É análogo a classe e objeto.</details>
...
4. O que são camadas (layers) em uma image e por que importam?
...
<details><summary>Resposta</summary>Para compilar em um estágio com o SDK pesado e gerar a image final só com o runtime e os artefatos, resultando em image menor e com menos superfície de ataque.</details>
