...
related: [image-vs-container, kubernetes-objects, migrations]
tags: [cicd, pipeline, devops, deploy, automation]
sources:
...
CI/CD automatiza o caminho do código até a produção. Continuous Integration (CI) integra e valida cada mudança com build e tests automáticos a cada commit. Continuous Delivery mantém o software sempre pronto para implantar, com a liberação acionada por decisão humana; Continuous Deployment vai além e implanta automaticamente toda mudança aprovada. Importa porque reduz o risco e o tempo de delivery, transformando deploys em events rotineiros e seguros.

## Explicação detalhada

**Continuous Integration (CI)**: a prática de integrar mudanças com frequência (idealmente várias vezes ao dia) numa branch principal, com um pipeline que a cada commit faz checkout, restaura dependências, compila, roda os tests (ver [testing tools](../04-unit-tests/testing-tools.md)) e análise estática (ver [SonarQube](../07-quality-solid/sonarqube.md)). O objetivo é detectar problemas cedo, mantendo a base sempre integrável e verde.

**Continuous Delivery**: estende a CI garantindo que o artefato (por exemplo, uma [image de container](image-vs-container.md)) esteja sempre pronto para implantar em produção, passando por ambientes (dev, staging) automaticamente, mas com a promoção final para produção exigindo uma aprovação manual. O deploy é um clique, não um evento de risco.

**Continuous Deployment**: remove a aprovação manual. Toda mudança que passa por todos os estágios automatizados vai a produção sem intervenção. Exige forte confiança na suíte de tests, em métricas e em estratégias de rollback.
...
Um pipeline típico tem estágios encadeados: **build**, **test**, **análise/quality gate**, **empacotar artefato** (image para o registry), **deploy em staging**, e **deploy em produção** (manual ou automático). Migrations de database (ver [migrations](../03-ef-dapper-postgresql/migrations.md)) entram como um passo controlado antes ou junto do deploy.
...
O artefato imutável é a peça central: o mesmo build é promovido por todos os ambientes. Não se recompila por ambiente; promove-se exatamente o que foi testado, mudando apenas a configuração injetada (ver [ConfigMap e Secret](kubernetes-objects.md)). Isso garante que o que foi validado é o que vai para produção.

## Exemplos em C#

Pipeline GitHub Actions para uma aplicação .NET: build, tests e image (YAML):
...
- CI detecta problemas cedo e mantém a base saudável, ao custo de manter uma suíte de tests confiável e rápida; tests lentos ou instáveis (flaky) minam o pipeline.
- Continuous Delivery dá controle sobre quando liberar, útil para janelas e conformidade, mas mantém um passo manual. Continuous Deployment maximiza velocidade, ao custo de exigir altíssima confiança em automação e observability.
- Blue-green dá rollback instantâneo, ao custo de duplicar a infraestrutura durante a troca. Canary reduz o raio de impacto, ao custo de mais complexidade de routing e métricas.
...
Use CI em todo projeto: build e tests automáticos a cada commit são o mínimo. Use Continuous Delivery para manter o software sempre pronto a implantar com promoção controlada; adote Continuous Deployment quando a maturidade de tests, métricas e rollback permitir. Escolha a estratégia de deploy conforme o risco: rolling para o caso comum, blue-green ou canary para mudanças sensíveis. Promova sempre o mesmo artefato. Evite automação de deploy sem tests confiáveis e sem observability, pois automatiza também a propagação de defeitos.
...
<details><summary>Resposta</summary>Faz checkout, restaura, compila, roda os tests e a análise estática, detectando problemas cedo e mantendo a base sempre integrável.</details>
