---
id: azure-visao-geral
title: "Azure: Visão Geral para Backend .NET"
area: docker-k8s-cicd-azure
difficulty: iniciante
prerequisites: []
related: [objetos-kubernetes, ci-cd, redis]
tags: [azure, nuvem, app-service, aks, servicos]
sources:
...
Azure é a nuvem da Microsoft, com forte integração ao ecossistema .NET. Para um backend .NET, os serviços mais relevantes se agrupam em compute (onde a aplicação roda), dados (onde o estado vive), mensageria e integração, e plataforma (identidade, segredos, observabilidade). Conhecer as opções de compute e quando usar cada uma é o ponto de partida para projetar e operar sistemas na Azure.
...
- **Azure Container Apps**: serverless para containers, com escala automática (inclusive a zero) e suporte a microsserviços e eventos, sem gerenciar o cluster. Bom meio-termo entre App Service e Kubernetes.
- **Azure Kubernetes Service (AKS)**: Kubernetes gerenciado (ver [objetos do Kubernetes](objetos-kubernetes.md)), para quem precisa de controle e flexibilidade plenos de orquestração. Mais poder, mais responsabilidade operacional.
- **Azure Functions**: serverless orientado a eventos, executando funções sob demanda (HTTP, fila, timer), com cobrança por execução. Bom para tarefas event-driven e cargas intermitentes.
...
- **Azure Cosmos DB**: banco NoSQL distribuído globalmente, com consistência ajustável (ver [teorema CAP](../05-mensageria-distribuidos/teorema-cap.md)).
- **Azure Cache for Redis**: Redis gerenciado para cache (ver [Redis](../08-diferenciais/redis.md)).
- **Azure Blob Storage**: armazenamento de objetos para arquivos e blobs.

**Mensageria e integração:**

- **Azure Service Bus**: broker de mensagens corporativo com filas e tópicos, DLQ embutida (ver [dead letter queue](../05-mensageria-distribuidos/dead-letter-queue.md)).
- **Azure Event Hubs**: ingestão de eventos em alta escala, próximo do modelo de log (análogo ao Kafka).
- **Azure Event Grid**: roteamento de eventos para reações.

**Plataforma:**

- **Microsoft Entra ID** (antes Azure AD): identidade e autenticação.
- **Azure Key Vault**: cofre de segredos, chaves e certificados.
- **Azure Monitor / Application Insights**: observabilidade, métricas, logs e traces (ver [observabilidade](../08-diferenciais/datadog-observabilidade.md)).
...
A autenticação entre serviços evita segredos com **Managed Identity**: o recurso (por exemplo, um App Service) recebe uma identidade gerenciada pela Azure e a usa para acessar Key Vault, banco ou storage sem credenciais no código. É o caminho recomendado, eliminando senhas em configuração.
...
- Functions são ótimas para eventos e cargas intermitentes, mas têm restrições (cold start, limites de execução) que as tornam inadequadas para tudo.
- Serviços gerenciados reduzem operação ao custo de menos controle e de lock-in parcial à plataforma.
...
- Esquecer health checks exigidos pela plataforma para roteamento e auto-recuperação.
...
<details><summary>Resposta</summary>Uma identidade gerenciada pela Azure atribuída a um recurso, usada para acessar outros serviços (Key Vault, banco, storage) sem credenciais no código, eliminando segredos em configuração.</details>
...
<details><summary>Resposta</summary>Para cargas orientadas a eventos e intermitentes (HTTP, fila, timer), com cobrança por execução, ciente de restrições como cold start e limites de tempo de execução.</details>
