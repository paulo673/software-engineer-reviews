...
prerequisites: []
related: [ci-cd]
tags: [git, gitflow, trunk-based, branching, versioning]
sources:
...
Para integrar código de uma feature incompleta sem expô-la, o trunk-based usa **feature flags**: o código vai para o trunk, mas o caminho fica desligado por configuração até estar pronto. Isso separa **deploy** (o código está em produção) de **release** (a funcionalidade está ativa para usuários), permitindo deploys contínuos sem liberar features pela metade. Exige disciplina de manter o trunk sempre verde (tests passando) e de limpar flags antigas.

Gitflow funciona bem quando releases são events periódicos e versionados, com necessidade de manter várias versões. Trunk-based prospera com delivery contínua e uma única versão em produção (típico de SaaS e serviços web).
...
- Trunk-based minimiza conflitos e habilita delivery contínua, ao custo de exigir feature flags, alta coverage de tests e disciplina para manter o trunk sempre liberável.
- Feature flags desacoplam deploy de release e permitem tests A/B e rollout gradual, ao custo de complexidade no código e de dívida se as flags não forem removidas.
- A escolha depende do produto: distribuído e versionado tende a Gitflow; SaaS de delivery contínua tende a trunk-based.
...
- Trunk-based sem tests confiáveis: integrar direto no trunk sem rede de tests propaga defeitos rapidamente.
...
Use Gitflow para produtos com releases versionados explícitos, múltiplas versões em suporte e cadência periódica (software distribuído, on-premises). Use trunk-based para serviços web e SaaS com delivery contínua, apoiado em feature flags, tests fortes e CI/CD. Mantenha branches curtas em qualquer modelo. Evite Gitflow com branches longas (perde o sentido de CI) e evite trunk-based sem a rede de tests e a disciplina que ele pressupõe.
...
<details><summary>Resposta</summary>Trunk-based, porque a integração frequente numa única branch mantém o conjunto sempre liberável e reduz conflitos, que é a essência da integração e delivery contínuas.</details>
