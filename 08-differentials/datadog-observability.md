...
title: "Datadog and Observability: Logs, Metrics and Traces"
area: differentials
difficulty: intermediate
prerequisites: []
related: [ci-cd, azure-overview, circuit-breaker-retry]
tags: [observability, datadog, logs, metrics, traces, opentelemetry]
sources:
...
Observabilidade é a capacidade de entender o que acontece dentro de um sistema a partir do que ele emite. Apoia-se em três pilares: logs (events discretos com contexto), metrics (medidas numéricas agregadas ao longo do tempo) e traces (o caminho de uma requisição através dos serviços). Datadog é uma plataforma que coleta, correlaciona e visualiza os três. Importa porque, em sistemas distribuídos, sem observability você opera no escuro quando algo falha.

## Explicação detalhada

Monitoramento responde "está funcionando?" com base em perguntas conhecidas; observability permite investigar problemas que você não previu, fazendo perguntas novas sobre o comportamento do sistema. Os três pilares:

- **Logs**: registros de events discretos, com mensagem e contexto (timestamp, nível, IDs). Respondem "o que aconteceu exatamente neste ponto". Logs estruturados (JSON com campos) são pesquisáveis e correlacionáveis; logs de texto solto são difíceis de consultar em escala.
...
- Confundir monitoramento (perguntas conhecidas) com observability (investigar o desconhecido).
...
Use os três pilares em qualquer sistema de produção não trivial, instrumentando com OpenTelemetry para neutralidade de vendor e enviando ao backend (Datadog, Application Insights). Propague o trace id por toda a requisição, use logs estruturados e defina SLOs com alertas baseados em sintomas. Aplique sampling para controlar custo. Não há "quando evitar" observability em produção; o que se evita é o excesso sem critério (logar tudo, alertar em tudo), que custa caro e atrapalha mais do que ajuda.

## Perguntas de auto-teste

1. Quais são os três pilares da observability e o que cada um responde?
<details><summary>Resposta</summary>Logs (o que aconteceu em um ponto, events discretos), metrics (quanto e qual a tendência, medidas agregadas no tempo) e traces (por onde a requisição passou e onde gastou tempo).</details>

2. Qual a diferença entre monitoramento e observability?
<details><summary>Resposta</summary>Monitoramento responde perguntas conhecidas ("está no ar?"); observability permite investigar problemas não previstos, fazendo perguntas novas sobre o comportamento do sistema.</details>
...
```mermaid
flowchart TD
    APP[Aplicação instrumentada com OpenTelemetry] --> L[Logs: events com contexto]
