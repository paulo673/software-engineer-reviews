---
id: dotnet8-csharp12-new-features
title: ".NET 8 and C# 12 New Features"
area: csharp-dotnet
difficulty: intermediate
prerequisites: [record-class-struct]
related: [garbage-collector, pipeline-middleware]
tags: [dotnet8, csharp12, language, platform, new-features]
sources:
...
**Construtores primários em qualquer tipo**: antes restritos a records, agora classes e structs comuns podem declarar parâmetros no cabeçalho do tipo. Esses parâmetros ficam disponíveis em todo o corpo, úteis para injeção de dependência sem campos repetitivos. Diferente do record, eles não geram propriedades públicas automaticamente nem value equality.

**Expressões de coleção**: sintaxe unificada `[...]` para criar arrays, listas, spans e outros types de coleção, com o operador spread `..` para concatenar sequências. Substitui várias formas antigas (`new[] { }`, `new List<int> { }`) por uma só.

**Parâmetros default em lambdas**: lambdas agora aceitam valores padrão e `params`, como métodos.

**Alias de qualquer tipo com `using`**: `using` pode dar nome a qualquer tipo, incluindo tuplas, arrays e types genéricos, não só types nomeados.
...
**`TimeProvider`**: uma abstração de tempo que torna código dependente de relógio testável. Em vez de `DateTime.UtcNow`, injeta-se um `TimeProvider` e nos tests usa-se um fake controlável.
...
- Construtores primários reduzem boilerplate, mas escondem os campos e podem confundir quem espera o padrão clássico; em types complexos, o construtor tradicional ainda é mais claro.
...
- Esperar value equality de uma classe só por usar construtor primário: isso é do record, não da classe.
