...
title: "record, class e struct em C#"
area: csharp-dotnet
difficulty: iniciante
prerequisites: []
related: [garbage-collector, generics-variancia]
tags: [tipos, valor, referencia, igualdade, imutabilidade]
sources:
...
`class`, `struct` e `record` definem tipos, mas diferem em duas dimensões: semântica de valor versus referência e igualdade por referência versus por valor. `class` é tipo de referência com igualdade por referência. `struct` é tipo de valor (vive na stack ou inline) com igualdade por valor. `record` é açúcar do compilador que gera igualdade por valor e imutabilidade convenientes, podendo ser `record class` (referência) ou `record struct` (valor). Escolher certo afeta cópia, alocação e comparação.
...
**Tipo de valor (`struct`)**: a variável contém o dado diretamente. Atribuir copia todos os campos. Structs vivem na stack quando são locais, ou inline dentro do objeto que as contém. Igualdade padrão de struct compara os campos (igualdade por valor), embora a implementação padrão use reflection e seja lenta se não sobrescrita.

**`record`**: a partir do C# 9, um `record` (que é `record class` por padrão) gera automaticamente: `Equals`/`GetHashCode` por valor, `ToString` legível, desconstrução, e o método `with` para criar cópias com alterações. A partir do C# 10 existe `record struct`, que é um tipo de valor com as mesmas conveniências. Records favorecem imutabilidade via propriedades `init`.

A confusão comum: `record` não é uma terceira categoria de memória. Um `record class` continua sendo referência; um `record struct` continua sendo valor. O que o `record` muda é a igualdade (vira por valor) e a ergonomia (with, ToString, desconstrução).

## Por baixo dos panos

Um `record class` é compilado para uma `class` normal com membros gerados: um `EqualityContract`, sobrescritas de `Equals(object)` e `Equals(OtherRecord)`, `GetHashCode` combinando os campos, operadores `==`/`!=`, um construtor de cópia protegido e o `with` que o usa. A igualdade compara o tipo e cada campo.

`struct` não tem herança (não pode derivar de outra struct nem servir de base), sempre deriva implicitamente de `System.ValueType`, e tem um construtor sem parâmetros que zera os campos. Boxing acontece quando uma struct é tratada como `object` ou interface: ela é copiada para o heap dentro de um objeto, o que custa alocação e anula parte da vantagem do tipo de valor.

`readonly struct` garante imutabilidade no nível do tipo e permite ao compilador evitar cópias defensivas. `record struct` pode ser `readonly record struct` para o mesmo efeito.

## Exemplos em C#

Igualdade: class compara referência, record compara valor:
...
- `record class`: ótimo para DTOs, mensagens e value objects imutáveis onde igualdade por valor faz sentido. Mesma alocação de class.
- `record struct`: value object por valor com igualdade conveniente, sem alocação no heap, mas com as regras de cópia de struct.

## Pegadinhas e erros comuns

- Achar que `record` é tipo de valor. Não é: `record` padrão é `record class`, continua no heap.
...
- Mutar uma propriedade de um `record` esperando imutabilidade: use `init` e `with` para manter o valor imutável.

## Quando usar e quando evitar

Use `class` para entidades com identidade e ciclo de vida, agregados e serviços. Use `record` (class) para DTOs, eventos, comandos e value objects imutáveis onde comparar por conteúdo é natural. Use `struct` para valores pequenos, imutáveis e de alta frequência (coordenadas, dinheiro, identificadores) onde evitar alocação importa. Evite struct grande ou mutável, e evite `class` quando você só quer um agrupamento imutável de dados comparável por valor.

## Perguntas de auto-teste

1. `record` é tipo de valor ou de referência?
<details><summary>Resposta</summary>Depende: record (ou record class) é de referência; record struct é de valor. O record por si só não muda a categoria de memória, muda a igualdade e a ergonomia.</details>

2. Por que dois objetos `class` com os mesmos campos não são iguais por padrão?
<details><summary>Resposta</summary>Porque a igualdade padrão de class compara referências, não conteúdo. Eles só seriam iguais se você sobrescrevesse Equals e GetHashCode.</details>
...
<details><summary>Resposta</summary>É copiar a struct para um objeto no heap quando ela é tratada como object ou interface. Importa porque gera alocação e anula a vantagem de ser tipo de valor.</details>
...
6. Por que a igualdade padrão de uma struct pode ser lenta?
...
flowchart TD
    ...
    Q2 -->|Sim| Q3{Quer igualdade e ergonomia prontas?}
