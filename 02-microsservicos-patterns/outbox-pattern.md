...
difficulty: avancado
prerequisites: [idempotencia, cqrs]
related: [saga, idempotencia, semantica-entrega]
tags: [outbox, mensageria, consistencia, eventos, dual-write]
sources:
...
O Outbox pattern resolve o problema do dual-write: salvar dados no banco e publicar uma mensagem em um broker são duas operações em sistemas distintos que não compartilham transação, então uma pode falhar deixando o sistema inconsistente. A solução é gravar a mensagem em uma tabela outbox dentro da mesma transação do banco, e um processo separado lê essa tabela e publica no broker. Importa para garantir que evento e estado nunca divirjam.

## Explicação detalhada

O problema central: ao processar um pedido, você quer (1) gravar o pedido no banco e (2) publicar um evento `OrderCreated` no broker. Se gravar no banco e o broker cair antes de publicar, o estado existe mas ninguém é notificado. Se publicar primeiro e a gravação falhar, notificou-se algo que não aconteceu. Não há transação distribuída barata entre banco e broker que resolva isso de forma confiável, e mesmo two-phase commit traz custos e fragilidade.

O Outbox transforma dois sistemas em um. A escrita de negócio e o registro da mensagem ocorrem na **mesma transação do banco**, em uma tabela `outbox`. Como é uma transação local única, ou as duas acontecem ou nenhuma. O evento agora está garantidamente persistido junto ao estado.
...
- **Change Data Capture (CDC)**: ferramentas como Debezium leem o log de transações do banco e emitem as mudanças, sem polling.

Como o relay pode falhar entre publicar e marcar como enviado, a entrega é **at-least-once**: a mesma mensagem pode ser publicada mais de uma vez. Por isso o consumidor precisa ser idempotente (ver [idempotência](idempotencia.md)). Outbox garante "pelo menos uma vez", idempotência no consumidor completa o efeito "exatamente uma vez".
...
- Outbox garante consistência entre estado e eventos sem transação distribuída, usando só a transação local do banco. É a solução padrão para publicação confiável de eventos.
...
- Polling consome o banco periodicamente; CDC evita isso mas adiciona uma ferramenta e dependência do log do banco.
...
- Esquecer a idempotência no consumidor: como a entrega é at-least-once, eventos repetidos causarão efeito duplicado.
...
Use Outbox sempre que precisar publicar eventos de forma confiável a partir de uma mudança de estado no banco, que é o caso típico em arquiteturas orientadas a eventos e em [sagas](saga.md). Evite quando não há publicação de eventos (operação puramente local) ou quando a perda ocasional de uma notificação é aceitável e o custo do Outbox não se justifica. Nunca substitua Outbox por dual-write direto quando a consistência importa.

## Perguntas de auto-teste

1. Qual problema o Outbox pattern resolve?
<details><summary>Resposta</summary>O dual-write: salvar no banco e publicar no broker são operações em sistemas distintos sem transação compartilhada, então uma pode falhar deixando estado e evento inconsistentes.</details>

2. Por que gravar a mensagem na mesma transação do banco resolve isso?
...
<details><summary>Resposta</summary>É o processo que lê a tabela outbox e publica no broker. Pode ser um polling publisher (consulta periódica) ou usar Change Data Capture lendo o log de transações do banco.</details>

4. Qual semântica de entrega o Outbox oferece e o que isso exige do consumidor?
...
6. Por que não basta salvar no banco e depois publicar no broker em sequência?
