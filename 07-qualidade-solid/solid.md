...
prerequisites: []
related: [code-smells, design-patterns-classicos]
tags: [solid, design, oop, principios, manutencao]
sources:
...
**L - Liskov Substitution Principle (LSP)**: subtipos devem ser substituíveis por seus tipos base sem quebrar o comportamento esperado. Se `Quadrado` herda de `Retangulo` mas viola a expectativa de que largura e altura variam independentemente, ele quebra o LSP. O contrato do tipo base deve ser honrado pelos derivados.
...
- Achar que usar o contêiner de DI já é "fazer DIP" mesmo dependendo de tipos concretos: DIP é depender de abstrações, não só injetar.
