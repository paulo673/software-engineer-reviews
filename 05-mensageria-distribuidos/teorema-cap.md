...
difficulty: avancado
prerequisites: [semantica-entrega]
related: [semantica-entrega, isolamento-transacao, saga]
tags: [sistemas-distribuidos, cap, consistencia, disponibilidade, particao]
sources:
...
O teorema CAP afirma que um sistema distribuído não pode garantir simultaneamente Consistência, Disponibilidade e tolerância a Partição: na presença de uma partição de rede, é preciso escolher entre consistência e disponibilidade. Como partições são inevitáveis em sistemas reais, a escolha prática é entre CP (preferir consistência, recusando respostas durante a partição) e AP (preferir disponibilidade, aceitando dados possivelmente desatualizados). Importa para decidir como um sistema se comporta sob falha de rede.
...
Quórum é o mecanismo central de ajuste: com N réplicas, exigir que escritas confirmem em W nós e leituras consultem R nós, com R + W > N, garante que toda leitura veja a última escrita (consistência forte) ao custo de disponibilidade quando não há nós suficientes. Reduzir R e W aumenta disponibilidade e latência baixa, ao custo de poder ler dado desatualizado.
...
CAP é uma propriedade de arquitetura, não de código de aplicação, mas a escolha aparece em como a aplicação lida com a consistência. Exemplo: escolher nível de consistência ao ler de um banco distribuído (ilustrativo, API depende do cliente):
...
Lidar com consistência eventual na aplicação, lendo a própria escrita a partir do comando em vez de reler imediatamente uma réplica (ver [CQRS](../02-microsservicos-patterns/cqrs.md)):
...
- AP mantém o sistema respondendo sob partição, ao custo de poder servir dado desatualizado e exigir reconciliação. Bom para feeds, carrinhos, catálogos, onde disponibilidade vale mais que ver o último valor.
- Consistência forte traz mais latência e menor disponibilidade; consistência eventual traz baixa latência e alta disponibilidade, com a complexidade de lidar com dados temporariamente divergentes.
...
Escolha CP quando a correção do dado é inegociável e é aceitável recusar serviço sob partição: transações financeiras, controle de estoque, invariantes críticas. Escolha AP quando manter o sistema respondendo importa mais que ver o último valor: catálogos, feeds, sessões, carrinhos, telemetria. Use consistência ajustável para calibrar por operação. Em todo caso, projete a aplicação para conviver com a janela de consistência eventual quando optar por AP, e use [sagas](../02-microsservicos-patterns/saga.md) para consistência de negócio entre serviços.

## Perguntas de auto-teste

1. O que o teorema CAP afirma?
<details><summary>Resposta</summary>Que um sistema distribuído não pode garantir consistência, disponibilidade e tolerância a partição ao mesmo tempo; sob uma partição de rede, é preciso escolher entre consistência e disponibilidade.</details>
...
<details><summary>Resposta</summary>O CP recusa ou bloqueia para não servir dado inconsistente (mantém consistência, perde disponibilidade); o AP responde mesmo com dado possivelmente desatualizado (mantém disponibilidade, perde consistência).</details>
...
<details><summary>Resposta</summary>Com N réplicas, exigir R + W > N (leituras em R nós, escritas em W) garante leitura do último valor (consistência forte); reduzir R e W aumenta disponibilidade e baixa latência ao custo de possível leitura desatualizada.</details>
...
flowchart TD
    ...
    P -->|Sim| ESC{Escolher}
    ESC -->|Consistência| CP[CP: recusa servir dado obsoleto, perde disponibilidade]
