...
prerequisites: []
related: [delivery-semantics, rabbitmq-vs-kafka, idempotency]
tags: [messaging, dlq, rabbitmq, retry, resilience]
sources:
...
Uma Dead Letter Queue (DLQ) é uma fila para onde vão as mensagens que não puderam ser processadas com sucesso, depois de esgotadas as tentativas. Em vez de perder a mensagem ou ficar reprocessando para sempre uma que sempre falha (envenenando a fila), a DLQ a isola para análise e tratamento posterior. Importa porque dá resiliência e observability ao consumo de mensagens, separando falhas transitórias de falhas permanentes.

## Explicação detalhada

No consumo de mensagens, uma mensagem pode falhar por dois motivos:

- **Falha transitória**: o database estava indisponível, um serviço deu timeout. Tentar de novo provavelmente resolve.
- **Falha permanente (poison message)**: a mensagem está malformada, viola uma invariante, reference algo que não existe. Reprocessar nunca vai funcionar.
...
1. **Retry com limite**: tentar processar algumas vezes, idealmente com backoff (ver [circuit breaker and retry](../02-microservices-patterns/circuit-breaker-retry.md)), para absorver falhas transitórias.
...
A DLQ preserva a mensagem original e seus metadados (motivo da morte, contagem de tentativas, timestamp), o que é o que permite diagnosticar e reprocessar. O replay normalmente republica a mensagem na fila principal após a correção da causa, e a idempotency do consumidor (ver [idempotency](../02-microservices-patterns/idempotency.md)) garante que um eventual reprocessamento duplicado não cause efeito indevido.
...
- Replay exige idempotency no consumidor para não duplicar efeitos.
...
- Esquecer idempotency ao fazer replay: efeitos duplicados.
...
Use DLQ em todo consumo de mensagens de produção, combinada com retry limitado e backoff, e com monitoramento ativo da DLQ. Distinga falhas transitórias (retry) de permanentes (direto para a DLQ quando detectáveis). Garanta idempotency para permitir replay seguro. Não há cenário sério de messaging em que faça sentido abrir mão de uma DLQ; o que se evita é a DLQ sem observability, que dá falsa sensação de segurança.
...
5. Por que a idempotency importa ao reprocessar a DLQ?
<details><summary>Resposta</summary>Porque o replay pode reentregar uma mensagem que já teve efeito parcial; com idempotency, reprocessá-la não causa efeito duplicado.</details>
