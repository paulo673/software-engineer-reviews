...
tags: [cqrs, command, query, mediator, architecture]
sources:
...
1. **CQRS leve (mesmo database)**: apenas separa os types e handlers de command e query no código, lendo e escrevendo o mesmo database. É o caso mais comum e barato, frequentemente implementado com a biblioteca MediatR (padrão Mediator, ver [classic design patterns](classic-design-patterns.md)).
...
3. **CQRS com armazenamentos separados**: escrita e leitura em bancos diferentes (por exemplo, escrita em PostgreSQL, leitura em Elasticsearch ou Redis), sincronizados por events. Aqui surge a consistência eventual: o lado de leitura fica momentaneamente atrasado em relação à escrita.

CQRS é frequentemente confundido com Event Sourcing, mas são independentes. Event Sourcing guarda os events como fonte da verdade; CQRS só separa leitura de escrita. Pode-se usar um sem o outro.
...
Quando os armazenamentos são separados, a sincronização do modelo de leitura costuma vir de events de domínio publicados pela escrita. O produtor publica de forma confiável (ver [Outbox](outbox-pattern.md)) e um projetor consome os events e atualiza o modelo de leitura. Esse atraso entre escrever e o read model refletir é a consistência eventual, e precisa ser tratada na experiência do usuário (por exemplo, retornar o dado recém-escrito a partir do command, não relendo imediatamente o read model).
...
- Achar que CQRS obriga dois bancos: a forma mais comum usa o mesmo database, só separando o código.
...
<details><summary>Resposta</summary>Não. A forma mais comum usa o mesmo database, separando apenas os types e handlers no código. Armazenamentos separados são um nível mais avançado.</details>

3. Qual a diferença entre CQRS e Event Sourcing?
<details><summary>Resposta</summary>CQRS separa leitura de escrita; Event Sourcing guarda events como fonte da verdade. São independentes e podem ser usados juntos ou separados.</details>
