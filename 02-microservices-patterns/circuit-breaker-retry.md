...
title: "Circuit Breaker and Retry with Polly"
area: microservices-patterns
difficulty: intermediate
prerequisites: [classic-design-patterns]
related: [idempotency, api-gateway-service-discovery]
tags: [resilience, polly, retry, circuit-breaker, timeout]
sources:
...
Retry e circuit breaker são padrões de resiliência para chamadas a dependências que podem falhar (rede, serviços, database). Retry repete uma operação que falhou por motivo transitório; circuit breaker para de tentar quando o destino está claramente indisponível, evitando martelar um serviço caído e dando tempo para ele se recuperar. Em .NET, a biblioteca Polly (integrada via `Microsoft.Extensions.Http.Resilience`) implementa ambos.
...
Retry só é seguro em operações idempotentes (ver [idempotency](idempotency.md)). Repetir um `POST` não idempotente pode duplicar efeitos.
...
- Aplicar retry a operação não idempotente: duplica efeitos. Garanta idempotency antes.
...
2. Por que retry exige idempotency?
